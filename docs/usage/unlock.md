# <code>microbe-count <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>microbe-count <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking microbe-count pipeline output directory is fast and easy! In its most basic form, <code>microbe-count <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./microbe-count unlock [-h] --output OUTPUT
```
...
microbe-count unlock --output /data/$USER/output
```