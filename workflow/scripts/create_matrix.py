...
@About:
    ...
    of X input file that was provided; however,
    you can provide string to strip any suffices
    from the sample's filename. Also, the name of
    the input file's parent directory can be used
    instead of using the input filename name. See
    --use-parent-dir-as-sample-name for more info.
...
@Options
    ...
    --nan-values NAN_VALUES
                    ...
                    empty string, i.e ''.
    --use-parent-dir-as-sample-name
                    Use the parent directory name as the
                    sample name. By default, the name of
                    the file is used as the sample name.
                    This option overrides the default
                    behavior and it will use the name of
                    the file's parent directory as the
                    sample name in the resulting matrix.
                    For example, given these paths to
                    input files:
                        - salmon/sampleA/quant.sf
                        - salmon/sampleB/quant.sf
                        - salmon/sampleC/quant.sf
                    If this option is provided the output
                    matrix will contain a column for each
                    sample where the column names will
                    resolve to: sampleA,sampleB,sampleC.
...
def build_matrix(file, matrix_dict, join_index, parse_index, clean_suffix=None, use_parent_dir_as_sample_name = False):
    ...
    @param clean_suffix <str>:
        ...
        Default: None
    @param use_parent_dir_as_sample_name <bool>:
        Use parent directory name as sample name. The default behavior is
        to use the basename of the file.
        Default: False
    @return matrix_dict <dict[str][str]=[str]>:
        Matrix dictionary to create where:
            [sample][feature] = feature_value
	"""
    ...
    with open(file, 'r') as fh:
        ...
        if use_parent_dir_as_sample_name:
            # Use parent directory name as
            # the sample name, overrides the
            # default behavior which used the
            # basename of file
            sample = os.path.basename(os.path.dirname(os.path.abspath(file)))
...
def main():
    ...
    for file in sorted(args.input):
        ...
        matrix = build_matrix(
            ...
            use_parent_dir_as_sample_name=args.use_parent_dir_as_sample_name
        )
